...
if (FALSE) {
  ...
  install.packages(c("car", "caret","chemometrics", "class", "doParallel", "gbm", "ggplot2", "gmodels", "pROC", "randomForest", "RhpcBLASctl", "rpart", "rpart.plot", "SuperLearner"))
}
...
- [cloud computing](https://arxiv.org/abs/1707.07452), etc...  
...
Supervised machine learning algorithms learn a target function _f_ that best maps to a dependent/response/target/outcome variable `Y` based on independent/input/predictor variable(s) `X1, X2, X3,... Xn` from a set of [training data](https://en.wikipedia.org/wiki/Training,_test,_and_validation_sets). The algorithm then tries to predict `Y` using new test data for `X`. It is then evaluated using some sort of performance metric. These predictions are influenced by bias, variance, reducible error, and irreducible [errors](https://machinelearningmastery.com/gentle-introduction-to-the-bias-variance-trade-off-in-machine-learning/).  

- More simply, supervised machine learning algorithms train with known data to predict classification of new data to predict an output using some sort of inputs.  
- See [An Introduction to Statistical Learning, Chapters 1 and 2](http://www-bcf.usc.edu/~gareth/ISL/) to learn more.  

##### 3.1 Classification or regression?
**Classification** is used when the `Y` variable is discrete. Binary examples generally refer to a yes/no situation where a `1` is prediction of the "yes" category and `0` as the "no". Classification models the probability that the outcome variable is 1 based on the covariates: $Pr(Y = 1 | X)$. This can be extended to multi-level classification as well.  
...
- Accuracy  
- Mean squared error  
- True positives and true negatives (see:  https://uberpython.wordpress.com/2012/01/01/precision-recall-sensitivity-and-specificity)
...
##### 5.2 Specific goals
- Check out the [`car` package](https://cran.r-project.org/web/packages/car/index.html) and original [research paper](http://eml.berkeley.edu/~cle/e250a_f13/mroz-paper.pdf) for the Mroz dataset.  
