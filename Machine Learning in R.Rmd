...
##### Regression setup
Now we want to do something similar for our lone **REGRESSION task:** linear regression. There are forms of regression that can perform classification (such as `glm`), but the one we use here - `lm` - is not one of them. Hence, we can only predict a continuous/integer Y output variable. 
```{r}
# View pidd variable names
names(pidd)

# 4) Define Y for regression; store "age" in Y_reg
Y_reg = pidd$age
head(Y_reg, n = 20)
...
# 6) Finally, convert the "diabetes" column into numeric indicators - this will turn "diabetes" into two columns!
data_reg = data.frame(model.matrix( ~ . -1, data_reg)) 
head(data_reg) 
```
