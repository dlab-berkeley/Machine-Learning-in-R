...
> One additional preprocessing aspect to consider: datasets that contain factor (categorical) features should be expanded out into numeric indicators (this is often referred to as [one-hot encoding](https://hackernoon.com/what-is-one-hot-encoding-why-and-when-do-you-have-to-use-it-e3c6186d008f). You can do this manually with the `model.matrix` R function. This makes it easier to code a variety of algorithms to a dataset as many algorithms handle factors poorly (decision trees are the main exception). When we predict age in our regression example below, we will do this manually for good practice although functions like the `lm` function will internally expand factor variables such as the `diabetes` factor predictor into numeric indicators.  

- **k-nearest neighbor, decision tree, random forest, boosting, and SuperLearner algorithms:** we want to remove our Y variable from our dataframe of X features and store it as its own object then split the data into training and test sets. 
- **linear regression:** we will similarly remove our Y outcome from our dataframe of X predictors, but this time will both 1) split the data into training and test sets _and_ 2) we will train the same model on _all of the data_ using R's handy `predict` function to create a test set based on predicted values from all of the data. 

> NOTE: Keep in mind that training/test dataset splitting is common, but not always necessary. We will introduce you to cross-validation in the second half of this workshop. 
