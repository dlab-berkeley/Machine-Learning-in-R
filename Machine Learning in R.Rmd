...
##### Check out these pioneers of computation, artificial intelligence, behavior modeling, and proto-machine learning:
...
Be sure to check out the [DeepMind blog](https://deepmind.com/blog/understanding-agent-cooperation/) to learn more about modeling social complexity.  
...
In supervised machine learning, classification and regression are common distinctions. Classification should be used when your target Y variable is discrete, such as a binary Y outcome variable.  
...
In the biological sciences, a climate scientist might use past and present weather data to try and [predict future weather patterns](http://www.geosci-model-dev-discuss.net/gmd-2015-273/gmd-2015-273.pdf).  

In the social sciences, a researcher might choose to [predict voter turnout](http://pubs.aeaweb.org/doi/pdfplus/10.1257/jep.28.2.51) based on the number of people currently registered to vote and preliminary polling numbers.  

In publich health, machine learning applications are useful to [predict outbreaks of infectious disease](http://journals.plos.org/plosntds/article?id=10.1371/journal.pntd.0004549)  

[Text analysis](http://www.digitalhumanities.org/dhq/vol/3/2/000041/000041.html) and social network analysis figure heavily into [humanities research](http://www.digitalhumanities.org/dhq/vol/3/2/000044/000044.html). 
...
We will use the "Mroz" dataset from the ["car" R package](https://cran.r-project.org/web/packages/car/index.html) for the examples in this workshop. The Mroz dataset contains informaiton about women's participation in the workforce in 1975. Be sure to run `?Mroz` to read the variable descriptions. The original research article can be found [here](http://eml.berkeley.edu/~cle/e250a_f13/mroz-paper.pdf).  

The goal is to fit various algorithms individually to learn about their construction, and then compare them simultaneously using the "SuperLearner" R package. We are interested a binary outcome - whether or not a woman participated in the workforce in 1975 based on several predictors.  
...
str(data)
```
Note that our Y response variable (Y) and our X predictor variables (data) are now contained within separate objects. Also, note that the factors have been expanded out into indicators thanks to `model.matrix`.  

Because you know that for classification our outcome Y variable must be discrete, remember that `Y` ("lfp") is left as "factor" type.  

Split the dataset. 70% will be assigned to the training dataset, while the remaining 30% will be ascribed to the test dataset. 
```{r}
library(caret)
set.seed(1)
...
Create a new copy of the "Mroz" dataset called "data_scaled", which will contain scaled values with means equal to 0 and standard deviations equal to 1. 
...
1) split the data again, 2) fit the model again, and 3) examine the accuracy of predictions in the contingency table (again).  
...
# create a stratified random split
split_scaled <- createDataPartition(Y, p = 0.70, list = FALSE) 

train_scaled <- data_scaled[split_scaled, ] # partition training dataset
test_scaled <- data_scaled[-split_scaled, ] # partition test dataset

train_label_scaled <- Y[split_scaled] # partition training Y variable vector
test_label_scaled <- Y[-split_scaled] # partition test Y variable vector 
```
...
It also helps to investigate the cross-validated errors for multiple k-values at once to see which is ideal. First, let's tidy the transformed "data_scaled" data:
```{r}
grp <- Y
X <- data_scaled
k <- length(unique(grp))
...
### Challenge 1  
1. What is the optimal k-value from the cross-validation plot? What happens to the predictive accuracy when you switch k to this value?  
...
Remember that under the hood, `lm` and `glm` are one hot encoding factors to indicators:
```{r}
# view structure of Mroz
str(Mroz)

# define our continuous Y response variable, "lwg"
Y2 <- Mroz$lwg
...
# in the help files.
dt <- rpart(Y ~ ., data = data, method = "class")  
...
rf <- data.frame(model.matrix( ~ . , subset(Mroz, select = -lfp)))
...
library(caret)
set.seed(1)
split_rf <- createDataPartition(Y, p=0.70, list=FALSE)
...
library(pROC)
set.seed(1)
gbm1 <- train(train_label ~ ., data = cbind(train_label = train_label, train), # cbind: caret expects the Y response and X predictors to be part of the same dataframe
	method = "gbm",
...
lfp <- Mroz$lfp
data <- data.frame(lfp, data_scaled)
str(data)
