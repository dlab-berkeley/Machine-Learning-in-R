...
Selecting a machine learning algorithm depends on the characteristics of the problem being investigated. Algorithms can be divided into three broad classes: supervised, unsupervised, and reinforcement - we will focus on two main subtypes of supervised machine learning: classification and regression.  

Supervised machine learning algorithms learn a target function _f_ that best maps to a dependent/response/target/outcome variable `Y` based on independent/input/predictor variable(s) `X1, X2, X3,... Xn`. The algorithm then tries to predict `Y` using new data for `X`. However, these predictions are influenced by bias, variance, and irreducible error. See [An Introduction to Statistical Learning, Chapters 1 and 2](http://www-bcf.usc.edu/~gareth/ISL/ISLR%20Sixth%20Printing.pdf) to learn more.  
...
- **`k`** is the number of neighbors used to classify the point and choosing a proper "k" is integral to finding the best-performing model.  
- **Large k-values** are bad because the class with the largest size might win regardless of the surrounding points.  
...
- `data` contains the expanded predictor variables.  
