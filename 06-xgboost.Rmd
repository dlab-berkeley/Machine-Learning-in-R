...
xg_fit_viz_metr <- visualize_metrics(xg_fit)
...
xg_fit_viz_mat <- visualize_conf_mat(xg_fit)
...
xg_grid <- grid_regular(mtry(range = c(1, 10)),
                          min_n(range = c(2, 10)),
                          levels = 5)
...
# Optimal parameter
best_tree <- select_best(xg_res, "accuracy")
...
# Metrics 
(xg_fit_viz_metr + labs(title = "Non-tuned")) / (visualize_metrics(xg_fit_tuned) + labs(title = "Tuned"))

# Confusion matrix 
(xg_fit_viz_mat + labs(title = "Non-tuned")) / (visualize_conf_mat(xg_fit_tuned) + labs(title = "Tuned"))
