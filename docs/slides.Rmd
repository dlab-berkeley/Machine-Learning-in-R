...
* R utility packages: "remotes", "dplyr"  
...
1) **Lasso regression example:** How well can a person's age be predicted using the other diabetes-related variables?  

2) **Binary classification examples:** How reliably can different algorithms predict a person's diabetes status using the other variables?  
...
2. Remove Y variables from X predictor datasets  
3. Convert factors to indicators (if necessary)  
4. Define training and test datasets  

---

# Cross-validation

.center[![desctipion of image](K-fold_cross_validation_EN.jpg)]

.center[[4 fold cross-validation image by Fabian Flöck - Own work, CC BY-SA 3.0](https://commons.wikimedia.org/w/index.php?curid=51562781)]
...
This algorithm ["regresses"](https://stats.stackexchange.com/questions/207425/why-do-we-say-the-outcome-variable-is-regressed-on-the-predictors/207621) $Y$ onto $X$. Although individual examples may differ (you could regress $X$ onto $Y$), in simpler terminology we want to see how our $X$ variables can be used to predict $Y$ for this example. 

Regression allows us to ask useful questions of our data, such as: 
1. Is there a relationship between $Y$ and $X$ variables? 
...
Linear regression can be thought of as the simplest form of supervised machine learning and can be used for both inference and prediction.  
...
Y = dependent/outcome/target/response variable  
X = independent/predictor/input/explanatory variable  
epsilon = error  
β0 = intercept  
...
[Ordinary least squares (OLS)](https://en.wikipedia.org/wiki/Ordinary_least_squares) regression is the simplest use case when the target $Y$ variable is continuous. OLS computes a best fit line that minimizes the vertical squared and summed distances from each point to calculate the model's goodness of fit. Our refresher example will predict age from the [Pima Indians Diabetes Database](https://www.kaggle.com/uciml/pima-indians-diabetes-database) (abbreviated as "pidd") using the other variables as predictors.  
