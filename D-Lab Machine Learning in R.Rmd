...
> Note: see the help files `?impute_missing_values` to see that the data are centered and scaled when "knn" is specified as the argument to the "type" parameter. 
...
```{r}
gmodels::CrossTable(x = test_label, y = knn_result, chisq = FALSE,
...
mean(test_label == knn_result) # ~ 0.71
```
How did it do?  
...
Remember that our knn imputation from the `ck37::impute_missing_values` function above has already centered and scaled our data for us. 

##### 1.3 knn - improving performance metrics - change k-value

On this particular dataset however, know that scaling the data does not help performance improve by any significant amount in its current configuration.  

> Note: More practice is always good for data preprocessing and splitting. In your own time, consider repeating all steps above, except use "standard" instead of "knn" as the method inside `impute_missing_values` function in the "Handling missing data" section above.  
...
```{r prob_hist}
mean(predicted_label == test_label) # ~0.73
...
OLS can be used when the target Y variable is continuous. Remember that under the hood, `lm` is one-hot encoding factors to indicators, but try writing it out for practice. 
