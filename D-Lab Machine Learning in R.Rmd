...
```{r, eval = F}
if (FALSE) {
  # Run these lines manually to install the necessary packages.
  install.packages(c("caret","chemometrics", "class", 
                  "devtools", "dplyr", "gbm", "ggplot2", "gmodels", 
...
> NOTE: Keep in mind that training/test dataset splitting is common, but not always preferred. We will introduce you to cross-validation in the second half of this workshop where _all_ the data is used and multiple training/testing splits are utilized. 
...
cv_sl = CV.SuperLearner(Y = y_int, X = subset(pidd, select = -diabetes),
                         SL.library = sl_lib,
                         family = binomial(),
                         cvControl = list(V = 5L),
                         verbose = TRUE)
