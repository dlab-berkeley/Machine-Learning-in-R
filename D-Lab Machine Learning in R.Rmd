...
Median imputation is also useful, but k-nearest neighbor (knn) imputation is demonstrated below:
...
> Note: view the help files for `?impute_missing_values` to see that the data are centered and scaled when "knn" is specified as the argument to the "type" parameter. You can also center and scale your data manually using R's builtin `scale` function - type `?scale` to learn more.  
...
set.seed(1)
knn_result = class::knn(train = train_x, test = test_x,
...
```{r}
gmodels::CrossTable(x = test_label, y = knn_result, 
                    # Unfortunately, proportions default to TRUE so we can set them to FALSE to get simpler results. 
                    chisq = FALSE,
                    prop.chisq = FALSE, 
                    prop.r = FALSE, 
                    prop.c = FALSE, 
...
Remember that our knn imputation from the `ck37::impute_missing_values` function above has already centered and scaled our data for us so we can focus on changing the value of `k`. 

> Note: More practice is always good for data preprocessing and splitting. In your own time, consider repeating all steps above, except use "standard" instead of "knn" as the method inside `impute_missing_values` function in the "Handling missing data" section above. Do your results change? Why or why not? 

##### 1.3 knn - improving performance metrics - change k-value

It also helps to investigate the cross-validated accuracy for multiple k-values at once to see which is ideal. Plot the cross-validated error rates:

```{r cross-validated k}
set.seed(1)
knn_k = chemometrics::knnEval(X = subset(pidd, select = -diabetes), grp = y_fac, train = training_rows, 
...
tree_pruned2 = prune(tree, cp = 0.047) # 3 splits

tree_pruned14 = prune(tree, cp = 0.011) # 14 splits
