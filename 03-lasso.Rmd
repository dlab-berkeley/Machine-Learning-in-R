...
# OLS spec 
ols_spec <- linear_reg() %>% # Specify the model 
  set_engine("lm") %>% # Specify the engine: lm, glmnet, stan, keras, spark 
  set_mode("regression") # Declare the mode: regression or classification 
...
- The following two visualizations draw on Julia Silge's [blog post](https://juliasilge.com/blog/lasso-the-office/). 
...
- Show the lambda that results in the minimum estimated mean-squared error (MSE):
  ...
  - The RMSE of the initial model: 7.858063
  - The RMSE of the tuned model: 7.760019
  
```{r}

best_rmse <- show_best(rec_res, metric = "rmse")

lowest_rmse <- select_best(rec_res, metric = "rmse")
...
finalize_workflow(
  ...
  ggplot(aes(x = abs(Importance), 
             y = fct_reorder(Variable, abs(Importance)),
             fill = Sign)) +
  geom_col() +
  labs(x = "Importance",
       y = NULL) 
