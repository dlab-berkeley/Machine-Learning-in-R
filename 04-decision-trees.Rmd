...
# Trees of similar sizes might appear to be tied for lowest "xerror", but a tree with fewer splits might be easier to interpret. However, a tree with 4 and 7 splits have lower relative errors. 
tree_pruned2 = prune(tree, cp = 0.031250 ) # 2 splits
tree_pruned4 = prune(tree, cp = 0.024306) # 4 splits
...
**Big question 2:** What do you notice about the tree with 2 splits and the tree with 4 splits? Are there any parts that are identical?  

##### Challenge 2
What are the "minsplit", "cp", and "minbucket" hyperparameters within the "control" function? Use the iris dataset to construct a decision tree that utilizes the `rpart.control` hyperparameter.  
