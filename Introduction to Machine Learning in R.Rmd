...
```{r, eval=FALSE}
install.packages(c("car", "caret","class","gbm", "ggplot2", "gmodels", "parallel", "pROC", "randomForest", "RhpcBLASctl", "rpart","SuperLearner"), dependencies=TRUE)
```
```{r}
library(car)
library(caret)
...
Machine learning algorithm selection depends on the nature of the problem being investigated. Algorithms are generally divided into three types: supervised, unspervised, and reinforcement - we will focus on supervised learning methods for this workshop. Classification and regression are examples of supervised learning.  

Supervised machine learning algorithms learn a target funciton _f_ that best maps to a response/target/outcome/dependent variable "Y" based on independent/predictor variable(s) "X". It then tries to predict Y based on new data for X. However, these predictions are influenced by three types of error: bias, variance, and irreducible error. See [An Introduction to Statistical Learning, pp. 18, 33-37.](http://www-bcf.usc.edu/~gareth/ISL/ISLR%20Sixth%20Printing.pdf) for definitions.  
...
It should again be emphasized that a major strength of machine learning is its pratical application for investigative research. Before performing machine learning in R, one must consider the nature of the problem being investigated. Some examples include:  
...
Some common performance metrics include accuracy, true positive/true negative rates, area under the ROC curve, and cross-validation. See the below links for more information:  
...
library(caret)
set.seed(1)
split_knn <- createDataPartition(Mroz_knn$lfp, p=0.70, list=FALSE) # create a stratified random split
...
Two common ways of improving kNN performanc are to 1) standardize the data so that scale does not unduly influence classification, and 2) change the k-value. 
...
Mroz_knn_scaled[,-1] <- scale(Mroz_knn[,-1], center=TRUE, scale=TRUE)
...
library(caret)
set.seed(1)
split_knn_scaled <- createDataPartition(Mroz_knn_scaled$lfp, p=0.70, list=FALSE) # create a stratified random split
...
train_labels_knn_scaled <- train_knn_scaled[,1] # partition training Y variable vector
test_labels_knn_scaled <- test_knn_scaled[,1] # partition test Y variable vector 
```
...
library(class)
set.seed(1)
Mroz_pred_knn_scaled <- knn(train=train_knn_scaled[,-1], test=test_knn_scaled[,-1], cl=train_labels_knn_scaled, k=23, prob=TRUE)
```
...
Now, we can fit a model that attempts to predict the binary Y variable labor force participation ("lfp") via the other X predictor variables. Mean squred error (MSE) will be our performance metric. MSE measures the difference between observed and expected values, with smaller values tending to reflect greater predictive accuracy. 
```{r}
...
#pdf("classification tree.pdf", 6,6)
par(mar=c(0,0,1,0))
...
#8. Random forests
Random forests also are recursive partitioning methods that use multiple decision trees for ensemble predictions for classification and regression. Unlike decision trees, by default results generally do not require pruning and include accuracy and variable importance information. Furthermore, at each random forest tree split, only a small portion of the predictors are used (rather than the full suite).  
...
Rather than testing only a single model at a time, it is useful to compare tuning parameters within that single model. Bootstrap is the default resampling method, but we want cross-validation. We also want to specify different tuning parameters.  

Let us first create two objects - `control` and `grid`. `control` will allow us to tune the cross-validated performance metric, while `grid` let's us evaluate the model with different characteristics: 
```{r}
control <- trainControl(method="repeatedcv", # choose 10-fold repeated measure cross-validation as our performance metric (instead of the default "bootstrap")
	...
	classProbs=TRUE, # instruct R to produce the class probabilities
...
#pdf("gbm tuning comparison.pdf", 9, 6)
ggplot(gbm1) + theme_bw() + ggtitle("GBM model comparisons") # plot the CV ROC accuracy of the tuned models
#dev.off()
```

#9. Ensemble methods
The ["SuperLearner" R package](https://cran.r-project.org/web/packages/SuperLearner/index.html) is a method that simplifies ensemble learning by allowing you to simultaneously evaluate the cross-validated performance of multiple algorithms/an algorithm with differently tuned parameters.  

Let's see how the five algorithms you learned in this workshop (kNN, linear regression, decision tree, random forest, and gradient boosted machine) compare to each other and to the mean of Y as a benchmark algorithm, in terms of their cross-validated error!

Check out the different algorithm wrappers offered by SuperLearner:
```{r}
listWrappers()
```
...
split_SL <- createDataPartition(data$lfp, p=0.70, list=FALSE) # create a stratified random split
train_SL <- data[split_SL, ] # partition training dataset

train_SL <- sample(train_SL[274:336,]) # choose less rows to speed up model fitting

Y <- ifelse(train_SL$lfp=="yes", 1, 0) # designate Y outcome variable

train_SL <- train_SL[,-1] # remove Y outcome variable

SL_library <- c("SL.mean", "SL.gbm", "SL.knn", "SL.glm", "SL.rpart", "SL.randomForest")
```
...
CV_SL <- CV.SuperLearner(Y=Y, X=train_SL, SL.library=SL_library, family=binomial(), parallel="multicore")
CV_SL$time
```

Risk will be the performance estimate. The lower the risk, the fewer errors the model makes in its prediction. The coefficient displays the importance of each algorithm in the overall ensemble. View and plot results:
```{r}
summary(CV_SL)
