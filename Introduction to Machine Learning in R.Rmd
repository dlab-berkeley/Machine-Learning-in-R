...
Mroz_knn = subset(Mroz_knn, select = -lfp)
...
str(Mroz_knn)
```
Note that our Y response variable (Y_knn) and our X predictor variables (the other variables) are now contained within separate objects! Also, note that the factors have been expanded out into indicators thanks to `model.matrix`.
...
Mroz_reg <- data.frame(model.matrix( ~ . - 1, subset(Mroz, select = -lwg)))
...
Mroz_reg_model <- lm(lfp ~ ., data = Mroz_reg)
...
# although we could use the "Mroz_knn" or "Mroz_reg", dataframes, it is good practice to again review what `model.matrix` is doing here:
