...
```{r, eval=FALSE}
install.packages(c("car", "caret", "chemometrics", "class", "doParallel", "gbm", "ggplot2", "gmodels", "parallel", "pROC", "randomForest", "RhpcBLASctl", "rpart","SuperLearner"))
```
```{r}
library(car)
library(caret)
...
Mroz_knn <- data.frame(model.matrix( ~ . - 1, subset(Mroz, select = -lfp))) # this is a good boiler-plate way to convert factors to indicators ("one-hot encoding")
...
```{r}
Mroz_reg_model <- lm(Y_reg ~ . - 1, data=Mroz_reg) # fit the model

summary(Mroz_reg_model) # view output

reg_pred <- predict(Mroz_reg_model, Mroz_reg) # calculate predictions

MSE <- mean((Y_reg - reg_pred)^2) # calculate MSE 
...
png("classification tree.png", height=6, width=6, units="in", res=600)
par(mar=c(0,0,1,0))
...
summary(gbm1, las=2) # plot variable importance     
