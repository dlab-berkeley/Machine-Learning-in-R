...
# Argh! More practice to remember what model.matrix does:
Mroz_gbm <- data.frame(model.matrix( ~ . -1, subset(Mroz, select = -lfp)))
...
```{r}
set.seed(1)
gbm1 <- train(Y_gbm ~ ., data=train_gbm,
	method = "gbm",
...
rocCurve <- roc(response=test_gbm$lfp,
...
### Challenge 5
1. Using your Mroz dataset, fit a GBM model where "wc" is your Y variable. What happens to AUC?
