...
```{r}
str(Mroz_knn)
Mroz_knn$wc <- ifelse(Mroz_knn$wc == "yes", 1L, 0L)
Mroz_knn$hc <- ifelse(Mroz_knn$hc == "yes", 1L, 0L)
str(Mroz_knn)
```
...
library(class)
set.seed(1)
Mroz_pred_knn <- knn(train = train_knn[,-1], test = test_knn[, -1],
...
library(class)
set.seed(1)
Mroz_pred_knn_scaled <- knn(train = train_knn_scaled[, -1],
                            test = test_knn_scaled[,-1], 
...
#train_SL <- train_SL[274:336, ] # choose fewer rows to speed up model fitting

Y <- ifelse(train_SL$lfp=="yes", 1, 0) # designate Y outcome variable

train_SL <- train_SL[, -1] # remove Y outcome variable

# In the interest of time we are not doing GBM.
...
### Challenge 6. 
1. Did you get exactly the same results as your neighbors?
