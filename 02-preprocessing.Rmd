...
> Note: since our data are on different scales we will also want to center and scale these data using the `scale` or `ck37r::standardize` function if you select "standard" (median) imputation as the method. If you select "knn" as the method, values will be automatically centered and scaled. For now, we will work with the raw data - see the help files for the `scale` or `ck37r::standardize` function to learn more. 
